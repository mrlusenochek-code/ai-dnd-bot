...
def _run_two_pass_with_llm(
    *,
    ...
    final_out = out[0]
    return final_out, calls


def test_entity_guard_triggers_repair_for_new_unintroduced_name() -> None:
    final_out, calls = _run_two_pass_with_llm(
        draft_text="Черновик сцены.\nЧто делаете дальше?",
        final_text="Из толпы выходит Алек и молча смотрит на тебя.\nЧто делаете дальше?",
        repair_text="Из толпы выходит местный фермер по имени Алек и просит говорить тише.\nЧто делаете дальше?",
    )
    assert len(calls) == 3
    assert final_out == "Из толпы выходит местный фермер по имени Алек и просит говорить тише.\nЧто делаете дальше?"


def test_entity_guard_does_not_repair_when_name_is_introduced() -> None:
    final_out, calls = _run_two_pass_with_llm(
        draft_text="Черновик сцены.\nЧто делаете дальше?",
        final_text="Из тени появляется Алек — местный фермер с окраины.\nЧто делаете дальше?",
    )
    assert len(calls) == 2
    assert final_out == "Из тени появляется Алек — местный фермер с окраины.\nЧто делаете дальше?"


def test_entity_guard_does_not_repair_when_name_is_in_context() -> None:
    final_out, calls = _run_two_pass_with_llm(
        draft_text="Черновик сцены.\nЧто делаете дальше?",
        final_text="К тебе подходит Алек и указывает на закрытую дверь.\nЧто делаете дальше?",
        previous_gm_text="У окна уже сидит Алек и слушает разговор в зале.",
    )
    assert len(calls) == 2
    assert final_out == "К тебе подходит Алек и указывает на закрытую дверь.\nЧто делаете дальше?"
