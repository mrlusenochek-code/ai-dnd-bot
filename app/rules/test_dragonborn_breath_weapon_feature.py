...
def test_dragonborn_breath_weapon_is_persisted_from_ancestry_choice(monkeypatch) -> None:
    blue_features = _create_dragonborn(monkeypatch, "blue")
    blue_breath = ((blue_features.get("features") or {}).get("breath_weapon") or {})
    assert blue_breath.get("damage_type") == "lightning"
    assert blue_breath.get("save_ability") == "dex"
    assert (blue_breath.get("area") or {}).get("shape") == "line"
    assert str(blue_breath.get("recharge") or "").strip().lower() == "short_or_long_rest"
    assert (blue_features.get("choices") or {}).get("draconic_ancestry", {}).get("damage_type") == "lightning"
    assert "lightning" in {str(x).strip().lower() for x in (blue_features.get("resistances") or [])}

    green_features = _create_dragonborn(monkeypatch, "green")
    green_breath = ((green_features.get("features") or {}).get("breath_weapon") or {})
    assert green_breath.get("damage_type") == "poison"
    assert green_breath.get("save_ability") == "con"
    assert (green_breath.get("area") or {}).get("shape") == "cone"
    assert (green_features.get("choices") or {}).get("draconic_ancestry", {}).get("damage_type") == "poison"
    assert "poison" in {str(x).strip().lower() for x in (green_features.get("resistances") or [])}
