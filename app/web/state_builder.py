...
async def build_state(db: AsyncSession, sess: Session) -> dict:
        ...
        players_payload.append(
            {
                ...
                "revealed_node_ids": get_player_revealed_node_ids(sess, sp.player_id),
            }
        )
    ...
    for group_id, group in group_states.items():
        ...
        groups_payload[group_id] = {
            ...
            "destination_event_states": _group_destination_event_states_summary(group),
            ...
            "last_travel_event_outcome_summary": _group_last_travel_event_outcome_summary(group),
    ...
    current_group_navigation_options = (
        get_current_group_navigation_options(sess, player_id=sess.current_player_id)
        if sess.current_player_id
        else []
    )
    ...
    current_group_node_services = (
        get_current_group_node_services(sess, player_id=sess.current_player_id)
        if sess.current_player_id
        else []
    )
    ...
    current_group_last_camp_result = (
        get_current_group_last_camp_result(sess, player_id=sess.current_player_id)
        if sess.current_player_id
        else None
    )
    ...
    current_group_last_context_action_result = (
        get_current_group_last_context_action_result(sess, player_id=sess.current_player_id)
        if sess.current_player_id
        else None
    )
    current_group_route_access_states = (
        get_current_group_route_access_states(sess, player_id=sess.current_player_id)
        if sess.current_player_id
        else []
    )
    current_group_context_action_states = (
        get_current_group_context_action_states(sess, player_id=sess.current_player_id)
        if sess.current_player_id
        else []
    )
    ...
    current_group_last_arrival_result = (
        get_current_group_last_arrival_result(sess, player_id=sess.current_player_id)
        if sess.current_player_id
        else None
    )
    ...
    current_group_last_destination_event_result = (
        get_current_group_last_destination_event_result(sess, player_id=sess.current_player_id)
        if sess.current_player_id
        else None
    )
    current_group_current_node_destination_event_state = (
        get_current_group_current_node_destination_event_state(sess, player_id=sess.current_player_id)
        if sess.current_player_id
        else None
    )
    current_group_destination_event_states = (
        get_current_group_destination_event_states(sess, player_id=sess.current_player_id)
        if sess.current_player_id
        else []
    )
    ...
    current_group_node_visit_states = (
        get_current_group_node_visit_states(sess, player_id=sess.current_player_id)
        if sess.current_player_id
        else []
    )
    current_group_route_traversal_states = (
        get_current_group_route_traversal_states(sess, player_id=sess.current_player_id)
        if sess.current_player_id
        else []
    )
    ...
    current_group_active_region_pursuit = (
        get_current_group_region_pursuit(sess, player_id=sess.current_player_id)
        if sess.current_player_id
        else None
    )
    current_group_last_region_pursuit_result = (
        get_current_group_last_region_pursuit_result(sess, player_id=sess.current_player_id)
        if sess.current_player_id
        else None
    )
    ...
    current_group_current_node_progress = (
        get_current_group_current_node_progress(sess, player_id=sess.current_player_id)
        if sess.current_player_id
        else None
    )
    current_group_region_exploration_summary = (
        get_current_group_region_exploration_summary(sess, player_id=sess.current_player_id)
        if sess.current_player_id
        else None
    )
    current_group_region_frontier_summary = (
        get_current_group_region_frontier_summary(sess, player_id=sess.current_player_id)
        if sess.current_player_id
        else None
    )
    current_group_region_gateways = (
        get_current_group_region_gateways(sess, player_id=sess.current_player_id)
        if sess.current_player_id
        else []
    )
    current_group_primary_region_gateway = (
        get_current_group_primary_region_gateway(sess, player_id=sess.current_player_id)
        if sess.current_player_id
        else None
    )
    ...
    return {
        ...
        "game": {
            ...
            "current_group_last_destination_event_result": current_group_last_destination_event_result,
            "current_group_current_node_destination_event_state": current_group_current_node_destination_event_state,
            "current_group_destination_event_states": current_group_destination_event_states,
            ...
            "current_group_current_node_progress": current_group_current_node_progress,
            "current_group_region_exploration_summary": current_group_region_exploration_summary,
