...
def test_changeling_requires_choice_asi(monkeypatch) -> None:
    ...
    assert "Exactly 1 race ASI choice" in str(exc.value.detail)


def test_changeling_requires_two_skills(monkeypatch) -> None:
    _setup_create_mocks(monkeypatch)
    payload = _base_payload()
    payload["race_choices"] = {
    ...
    with pytest.raises(HTTPException) as exc:
        asyncio.run(http_routes.api_character_create(payload))
    assert exc.value.status_code == 400
    assert "Exactly 2 race skill choice" in str(exc.value.detail)


def test_changeling_requires_two_languages(monkeypatch) -> None:
    _setup_create_mocks(monkeypatch)
    payload = _base_payload()
    payload["race_choices"] = {
        "asi": [{"stat": "wis", "bonus": 1}],
    ...
    with pytest.raises(HTTPException) as exc:
        asyncio.run(http_routes.api_character_create(payload))
    assert exc.value.status_code == 400
    assert "Exactly 2 race language choice" in str(exc.value.detail)


def test_changeling_create_success_persists_choices_and_applies_asi(monkeypatch) -> None:
    _setup_create_mocks(monkeypatch)
    payload = _base_payload()
    payload["uid"] = 9732
    payload["race_choices"] = {
    ...
    assert choices.get("asi") == [{"stat": "wis", "bonus": 1}]
    assert choices.get("skills") == ["deception", "insight"]
    assert choices.get("languages") == ["elvish", "orc"]

    prof_skills = set(prof.get("skills") or [])
    assert {"deception", "insight"}.issubset(prof_skills)

    languages = set(race_features.get("languages") or [])
    assert {"common", "elvish", "orc"}.issubset(languages)
    assert isinstance(features.get("shapechanger"), dict)
