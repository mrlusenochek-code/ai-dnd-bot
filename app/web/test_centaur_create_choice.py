...
def test_centaur_create_requires_survivor_skill_choice(monkeypatch) -> None:
    _setup_create_mocks(monkeypatch)

    with pytest.raises(HTTPException) as exc:
        asyncio.run(http_routes.api_character_create(_base_payload()))
    assert exc.value.status_code == 400
    assert "Exactly 1 race skill choice" in str(exc.value.detail or "")


def test_centaur_create_persists_skill_choice_and_applies_asi(monkeypatch) -> None:
    _setup_create_mocks(monkeypatch)
    ...
    charge = features.get("charge") or {}
    assert int(charge.get("move_ft") or 0) == 30
    assert str(charge.get("activation") or "").strip().lower() == "bonus_action"
    assert str(charge.get("bonus_attack") or "").strip().lower() == "hooves"

    choices = race_features.get("choices") or {}
    assert choices.get("skills") == ["survival"]
    prof = race_features.get("proficiencies") or {}
    prof_skills = set(prof.get("skills") or [])
    assert "survival" in prof_skills
