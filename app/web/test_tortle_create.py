...
def test_tortle_create_without_extra_choices(monkeypatch) -> None:
    ...
    assert int(nat_armor.get("ac") or 0) == 17
    assert nat_armor.get("no_armor_stack") is True

    features = race_features.get("features") or {}
    claws = features.get("claws") or {}
    hold_breath = features.get("hold_breath") or {}
    shell_defense = features.get("shell_defense") or {}
    assert claws.get("damage_dice") == "1d4"
    assert claws.get("damage_type") == "slashing"
    assert hold_breath.get("duration") == "1_hour"
    assert int(shell_defense.get("ac_bonus") or 0) == 4
    assert shell_defense.get("adv_saves") == ["str", "con"]
    assert shell_defense.get("disadv_saves") == ["dex"]

    natural_weapons = race_features.get("natural_weapons") or []
    claw_profile = next((x for x in natural_weapons if str((x or {}).get("key") or "").strip().lower() == "claws"), {})
    assert claw_profile.get("damage_dice") == "1d4"

    runtime = race_features.get("runtime") or {}
    assert runtime.get("shell_defense_active") is False


def test_tortle_create_does_not_require_subrace_or_race_choices(monkeypatch) -> None:
    _setup_create_mocks(monkeypatch)
    response = asyncio.run(http_routes.api_character_create(_payload()))
    assert response.status_code == 200


def test_tortle_ui_texts_present_in_templates() -> None:
    session_template = (Path(__file__).resolve().parents[0] / "templates" / "session.html").read_text(encoding="utf-8")
    assert "Природная броня: КД" in session_template
    assert "Когти: 1d4 рубящего урона" in session_template
    assert "Задержка дыхания: до 1 часа" in session_template
    assert "Выживание: владение навыком" in session_template
    assert "Защита панцирем доступна: действием спрятать конечности в панцирь." in session_template
    assert "Защита панцирем активна: КД +4, преимущество к спасброскам Силы и Телосложения, помеха к спасброскам Ловкости, скорость 0." in session_template
