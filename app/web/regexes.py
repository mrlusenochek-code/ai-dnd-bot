...
CHAT_COMBAT_ACTION_PATTERNS: list[tuple[str, re.Pattern[str]]] = [
    ...
    (
        "combat_indomitable",
        re.compile(r"(использую\s+несгибаем\w*|несгибаем\w*|indomitable)", re.IGNORECASE),
    ),
    (
    ...
    (
        ...
        re.compile(
            r"(втор\w+\s+рук\w*|удар\w*\s+втор\w+\s+рук\w*|атак\w*\s+втор\w+\s+рук\w*|off-?hand)",
            re.IGNORECASE,
        ),
    ),
    (
    ...
    (
        ...
        re.compile(r"(хитр\w*\s+действ\w*\s+(?:отхож\w*|отход\w*|отступ\w*|отступаю|отхожу)|бонусн\w*\s+действ\w*\s+(?:отступаю|отступ\w*|отхожу|разрыва\w+\s+дистанц\w*)|cunning\s+action\s+disengage)", re.IGNORECASE),
    ),
    (
        "combat_cunning_hide",
        re.compile(r"(хитр\w*\s+действ\w*\s+(?:пряч\w*|скрываюсь|засад\w*)|бонусн\w*\s+действ\w*\s+(?:пряч\w*|скрываюсь|скрыт\w*|засад\w*)|cunning\s+action\s+hide)", re.IGNORECASE),
    ),
    (
        "combat_shift",
    ...
    (
        "combat_attack",
        re.compile(
            r"(атак|напад|удар|бью|рубл|колю|выпад|тыч|пыр|замах|метаю|швыряю|стреля|выстрел|стрел|лук|арбалет|режу|вступаю\s+в\s+бой|вступить\s+в\s+бой|вхожу\s+в\s+бой|войти\s+в\s+бой|врываюсь\s+в\s+бой|двумя\s+оруж\w*|с\s+двух\s+рук|двумя\s+кинжал\w*|удар\s+двумя\s+руками)",
            re.IGNORECASE,
        ),
    ),
    (
    ...
    (
        "combat_jump",
        re.compile(r"(\bjump\s+(?:long|high)\b|прыга\w*\s+в\s+длин\w*|прыж\w*\s+в\s+длин\w*|прыга\w*\s+в\s+высот\w*|прыж\w*\s+в\s+высот\w*)", re.IGNORECASE),
    ),
    (
