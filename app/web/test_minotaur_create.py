...
def _base_payload() -> dict[str, Any]:
    return {
        ...
        "race_choices": {"skills": ["intimidation"]},
    }


def test_minotaur_create_requires_one_skill_and_persists_features(monkeypatch) -> None:
    ...
    assert (race_features.get("languages") or []) == ["common", "minotaur"]

    natural_weapons = race_features.get("natural_weapons") or []
    horns = next((x for x in natural_weapons if str((x or {}).get("key") or "").strip().lower() == "horns_minotaur"), {})
    assert str(horns.get("damage_dice") or "").strip().lower() == "1d6"
    assert str(horns.get("damage_type") or "").strip().lower() == "piercing"
    assert str(horns.get("ability") or "").strip().lower() == "str"

    prof = race_features.get("proficiencies") or {}
    assert "intimidation" in (prof.get("skills") or [])

    features = race_features.get("features") or {}
    assert isinstance(features.get("goring_rush"), dict)
    assert isinstance(features.get("hammering_horns"), dict)

    choices = race_features.get("choices") or {}
    assert (choices.get("skills") or []) == ["intimidation"]


def test_minotaur_create_rejects_missing_or_invalid_skill(monkeypatch) -> None:
    _setup_create_mocks(monkeypatch)
    ...
    payload_bad["race_choices"]["skills"] = ["arcana"]
    try:
...
def test_minotaur_combat_action_detection() -> None:
    assert _detect_chat_combat_action("пронзающий натиск") == "combat_goring_rush"
    assert _detect_chat_combat_action("goring rush") == "combat_goring_rush"
    assert _detect_chat_combat_action("сокрушительные рога") == "combat_hammering_horns"
    assert _detect_chat_combat_action("hammering horns") == "combat_hammering_horns"


def test_minotaur_ui_texts_present_in_session_template() -> None:
    template = (Path(__file__).resolve().parents[0] / "templates" / "session.html").read_text(encoding="utf-8")
    assert "Рога: природное оружие 1к6 + СИЛ колющий" in template
    assert "Впечатляющий вид: владение навыком" in template
    assert "Пронзающий натиск: после Рывка и 20 фт" in template
