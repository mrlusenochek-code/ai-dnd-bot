...
def test_verdan_race_features_persist_on_create(monkeypatch) -> None:
    ...
    assert str(race_features.get("size") or "") == "small"
    ...
    save_advantages = saves.get("advantage") or []
    assert "wis" in save_advantages
    assert "cha" in save_advantages

    features = race_features.get("features") or {}
    size_change = features.get("size_change") or {}
    assert int(size_change.get("at_level") or 0) == 5
    assert isinstance(features.get("hit_dice_reroll"), dict)

    proficiencies = race_features.get("proficiencies") or {}
    prof_skills = proficiencies.get("skills") or []
    assert "persuasion" in prof_skills
