...
def test_triton_create_without_extra_choices(monkeypatch) -> None:
    ...
    assert langs == ["common", "primordial"]

    senses = race_features.get("senses") or {}
    assert int(senses.get("darkvision_ft") or 0) == 60

    assert "cold" in {str(x).strip().lower() for x in (race_features.get("resistances") or [])}

    features = race_features.get("features") or {}
    assert features.get("amphibious") is True
    guardians = features.get("guardians_of_the_depths") or {}
    emissary = features.get("emissary_of_the_sea") or {}
    assert guardians.get("cold_resistance") is True
    assert guardians.get("deep_pressure_adapted") is True
    assert emissary.get("scope") == "sea_beasts"

    innate_spells = race_features.get("innate_spells") or []
    names = [str((x or {}).get("name") or "").strip().lower() for x in innate_spells if isinstance(x, dict)]
    assert names == ["create_or_destroy_water", "gust_of_wind", "wall_of_water"]

    runtime = race_features.get("runtime") or {}
    assert runtime.get("triton_gust_of_wind_used") is False
    assert runtime.get("triton_wall_of_water_used") is False
    assert runtime.get("triton_active_water_wall") is None


def test_triton_create_does_not_require_subrace_or_race_choices(monkeypatch) -> None:
    _setup_create_mocks(monkeypatch)
    response = asyncio.run(http_routes.api_character_create(_payload()))
    assert response.status_code == 200
