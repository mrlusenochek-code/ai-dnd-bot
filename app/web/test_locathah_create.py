...
def test_locathah_create_persists_expected_race_features(monkeypatch) -> None:
    ...
    assert (race_features.get("languages") or []) == ["common", "aquan"]
    ...
    assert str(nat_armor.get("ac_formula") or "").strip().lower() == "12 + dex_mod"

    prof = race_features.get("proficiencies") or {}
    prof_skills = [str(x).strip().lower() for x in (prof.get("skills") or [])]
    assert "athletics" in prof_skills
    assert "perception" in prof_skills
...
def test_locathah_ui_texts_present_in_session_template() -> None:
    template = (Path(__file__).resolve().parents[0] / "templates" / "session.html").read_text(encoding="utf-8")
    assert "Природный доспех: без доспеха КД = 12 + модиф. ЛОВ" in template
    assert "Воля Левиафана: преимущество на спасброски от испуга/яда/очарования/ошеломления/паралича/усыпления" in template
    assert "Частичная земноводность: нужно погружаться в воду раз в 4 часа, иначе начинаете задыхаться." in template
