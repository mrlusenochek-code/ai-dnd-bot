...
def test_kobold_create_applies_asi_features_and_languages(monkeypatch) -> None:
    ...
    payload = {
        ...
        "race_id": "kobold",
        "subrace_id": "",
        "stats": {"str": 5, "dex": 50, "con": 50, "int": 50, "wis": 50, "cha": 50},
        "race_choices": {},
    }
    ...
    assert int(stats.get("str") or 0) == 0

    senses = race_features.get("senses") or {}
    assert int(senses.get("darkvision_ft") or 0) == 60

    features = race_features.get("features") or {}
    assert isinstance(features.get("pack_tactics"), dict)
    assert isinstance(features.get("grovel_cower_beg"), dict)
    sunlight = features.get("sunlight_sensitivity")
    assert isinstance(sunlight, list)
    assert "attack_rolls" in sunlight
