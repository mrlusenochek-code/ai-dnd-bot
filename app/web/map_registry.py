...
STATIC_MAP_NODES: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "zone_band": "safe",
        "short_description": "Тихий береговой переход между трактом, городком и подъёмом к сторожевой башне.",
        "inspect_summary": "Отсюда хорошо читается береговая тропа: назад к безопасным дорогам, выше к старой башне.",
        "travel_note": "Удобная промежуточная точка, где легко сверить путь перед подъёмом к башне.",
        "service_hints": ["тихий привал у воды", "виден подъём к башне"],
        "visible_objects": [
            "узкая береговая тропа с мокрыми следами",
            "каменистый подъём к сторожевой башне",
            "низкие вехи между трактом и городком",
        ],
        "aliases": (
            "восточный берег",
            "берег",
...
STATIC_MAP_CONTEXT_ACTION_EFFECTS: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "label": "Принять arrival word",
        "action_kind": "support",
        "effect_type": "support",
        "one_shot": True,
        "result_type": "local_support_applied",
        "summary": "Пустить arrival-confirmed watch-road continuation в первый живой returned word о том, что resumed wayfarers действительно дошли дальше по reclaimed triangle.",
        "result_summary": "На постоялом дворе watch-road line к northwatch читают уже не только как arrival-confirmed continuation, а как leg, откуда приходит живое слово назад. Yard теперь не просто знает, что resumed wayfarers доходят дальше, а получает honest back-word, что линия держится: watch-road leg работает уже как roadside continuation with returned word, а не только как confirmed onward route.",
        "discovered_notes": [
            "На waystation_yard watch-road continuation уже не только arrival-confirmed, а и hears its own return word: назад приходит живое подтверждение, что resumed wayfarers реально дошли дальше."
        ],
        "applied_effects": ["frontier_arrival_word:northwatch_western", "intel:watchroad_arrival_word"],
        "node_state_flags": ["western_road_watchroad_arrival_word_received"],
        "node_state_summary": "На постоялом дворе уже приняли живое arrival word о том, что resumed wayfarers по watch-road continuation реально дошли дальше.",
    },
    {
        "node_id": "blackwater_run",
        "action_id": "gather_sidepass_arrival_word",
        "label": "Собрать arrival word",
        "action_kind": "support",
        "effect_type": "support",
        "one_shot": True,
        "result_type": "local_support_applied",
        "summary": "Пустить arrival-confirmed side-pass continuation в первый gathered back-word о том, что guided stragglers действительно прошли safer continuation reclaimed triangle.",
        "result_summary": "У чёрной протоки cautious side-pass к western_road читают уже не только как arrival-confirmed forward line, а как marsh leg, которая слышит своё continued hold назад. Blackwater_run теперь не просто знает, что guided stragglers проходят дальше, а собирает honest arrival word, что safer continuation действительно сработала: reeds-side leg работает уже как lived returned-word line, и этот back-sign worth carrying forward по cautious pass.",
        "discovered_notes": [
            "У blackwater_run safer side-pass уже не только arrival-confirmed, а и hears its own back-word: по marsh leg возвращается живое подтверждение, что guided stragglers реально прошли дальше."
        ],
        "applied_effects": ["frontier_arrival_word:deep_marsh_western", "intel:sidepass_arrival_word"],
        "node_state_flags": ["deep_marsh_sidepass_arrival_word_gathered"],
        "node_state_summary": "У чёрной протоки уже собрали живое arrival word о том, что guided stragglers по safer side-pass continuation реально прошли дальше.",
    },
    {
        "node_id": "ash_pass",
        "action_id": "catch_marsh_edge_return_word",
        "label": "Перехватить return word",
        "action_kind": "support",
        "effect_type": "support",
        "one_shot": True,
        "result_type": "local_support_applied",
        "summary": "Пустить arrival-confirmed wet-edge continuation в первый caught return word о том, что steadied traffic действительно влилось обратно в watch continuity reclaimed triangle.",
        "result_summary": "На ash_pass wet-edge return line к deep_marsh читают уже не только как arrival-confirmed continuation, а как boundary leg, откуда возвращается живое слово назад. Wet edge теперь не просто знает, что steadied traffic вливается дальше, а перехватывает honest return word, что watch continuity действительно приняла этот ход: marsh-edge leg работает уже как line that hears return continuity hold, а не только как confirmed edge-return route.",
        "discovered_notes": [
            "На ash_pass wet-edge continuation уже не только arrival-confirmed, а и hears its own return word: назад приходит живое подтверждение, что steadied traffic реально влилось в watch-side continuity."
        ],
        "applied_effects": ["frontier_arrival_word:northwatch_deep_marsh", "intel:marsh_edge_return_word"],
        "node_state_flags": ["northwatch_marsh_edge_return_word_caught"],
        "node_state_summary": "На ash_pass уже перехватили живое return word о том, что steadied traffic по wet-edge continuation реально влилось дальше в watch continuity.",
    },
    {
        "node_id": "forest_settlement",
    ...
    },
    {
        ...
        "label": "Сверить referral uptake",
        "action_kind": "support",
        "effect_type": "support",
        "one_shot": True,
        "result_type": "local_support_applied",
        "summary": "Признать reclaimed triangle как working lived continuation frontier fabric, по которой resumed traffic реально следует continuation cues на всех трёх legs.",
        "result_summary": "Когда watch-road reentry referral followed, reeds-side forward referral taken и marsh-edge return referral followed возвращаются домой уже как три referral-uptake field proofs, лесной посёлок видит reclaimed triangle не только как onward-referral fabric, а как working lived continuation / referral-uptake infrastructure. Relay road, cautious marsh pass и wet boundary watch теперь не просто выставляют dependable cues, а реально ведут resumed traffic по этим continuation lines: база помнит весь reclaimed triangle уже как practical lived onward-continuity fabric, на которую frontier действительно опирается.",
        "discovered_notes": [
            "Reclaimed triangle впервые remembered дома уже не только как onward-referral fabric, а как practical lived continuation / referral-uptake infrastructure: три field uptake proofs показывают, что resumed traffic реально следует continuation cues по всему loop."
        ],
        "applied_effects": ["frontier_reclaimed_referral_uptake:closed", "intel:frontier_reclaimed_referral_uptake_closed"],
        "node_state_flags": ["frontier_reclaimed_referral_uptake_closed"],
        "node_state_summary": "В лесном посёлке уже признали reclaimed triangle как working lived continuation frontier fabric, по которой resumed traffic реально следует continuation cues по всем трём reclaimed legs.",
    },
    {
        "node_id": "forest_settlement",
        "action_id": "review_reclaimed_arrival_confirmation",
        "label": "Сверить arrival confirmation",
        "action_kind": "support",
        "effect_type": "support",
        "one_shot": True,
        "result_type": "local_support_applied",
        "summary": "Признать reclaimed triangle как working arrival-confirmed frontier fabric, по которой resumed traffic не только ведут, но и устойчиво подтверждают дальше по всем трём legs.",
        "result_summary": "Когда watch-road reentry arrivals confirmed, reeds-side forward arrivals confirmed и marsh-edge return arrivals confirmed возвращаются домой уже как три arrival-confirmation field proofs, лесной посёлок видит reclaimed triangle не только как lived continuation fabric, а как working confirmed onward-continuity infrastructure. Relay road, cautious marsh pass и wet boundary watch теперь не просто ведут resumed traffic по continuation cues, а реально подтверждают, что этот traffic доходит дальше по линии: база помнит весь reclaimed triangle уже как practical arrival-confirmed continuation fabric, на которую frontier действительно опирается.",
        "discovered_notes": [
            "Reclaimed triangle впервые remembered дома уже не только как lived continuation fabric, а как practical arrival-confirmed continuation infrastructure: три field confirmation proofs показывают, что resumed traffic реально доходит дальше по всему loop."
        ],
        "applied_effects": ["frontier_reclaimed_arrival_confirmation:closed", "intel:frontier_reclaimed_arrival_confirmation_closed"],
        "node_state_flags": ["frontier_reclaimed_arrival_confirmation_closed"],
        "node_state_summary": "В лесном посёлке уже признали reclaimed triangle как working arrival-confirmed frontier fabric, по которой resumed traffic устойчиво подтверждают дальше по всем трём reclaimed legs.",
    },
    {
        "node_id": "forest_settlement",
...
STATIC_MAP_CONTEXT_ACTION_REQUIREMENTS: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        "node_id": "waystation_yard",
        "action_id": "receive_watchroad_arrival_word",
        "requires_node_state_flag": "western_road_watchroad_reentry_arrivals_confirmed",
        "requires_any_group_node_state_flags": ["frontier_reclaimed_arrival_confirmation_closed"],
        "unlock_hint": "Сначала реально признать reclaimed triangle как working arrival-confirmed frontier fabric дома и уже после этого принимать живое arrival word на watch-road leg.",
    },
    {
        "node_id": "blackwater_run",
        "action_id": "gather_sidepass_arrival_word",
        "requires_node_state_flag": "deep_marsh_sidepass_forward_arrivals_confirmed",
        "requires_any_group_node_state_flags": ["frontier_reclaimed_arrival_confirmation_closed"],
        "unlock_hint": "Сначала реально признать reclaimed triangle как working arrival-confirmed frontier fabric дома и уже после этого собирать arrival word на cautious side-pass leg.",
    },
    {
        "node_id": "ash_pass",
        "action_id": "catch_marsh_edge_return_word",
        "requires_node_state_flag": "northwatch_marsh_edge_return_arrivals_confirmed",
        "requires_any_group_node_state_flags": ["frontier_reclaimed_arrival_confirmation_closed"],
        "unlock_hint": "Сначала реально признать reclaimed triangle как working arrival-confirmed frontier fabric дома и уже после этого перехватывать return word на мокрой edge-return leg.",
    },
    {
        "node_id": "northwatch_palisade",
        "action_id": "set_relay_watch",
...
STATIC_MAP_NODE_STATE_OVERLAYS: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "context_note": "В лесном посёлке reclaimed triangle уже remembered как working lived continuation frontier fabric, на которую реально опираются для taken reentry, taken forward и followed return continuity.",
        "detail_note": "Watch-road reentry referral followed, reeds-side forward referral taken и marsh-edge return referral followed уже сводятся дома не просто в onward-referral memory, а в одну practical lived continuation / referral-uptake infrastructure: settlement видит, что reclaimed triangle реально ведёт resumed traffic по dependable continuation lines на всех трёх legs.",
        "service_note": "После referral-uptake review база знает reclaimed triangle уже не только как onward-referral fabric, а как working lived continuation frontier fabric с честной памятью о taken continuation across the whole loop.",
    },
    {
        "node_id": "forest_settlement",
    ...
    {
        "node_id": "waystation_yard",
        "state_flag": "western_road_watchroad_arrival_word_received",
        "context_note": "На постоялом дворе уже держат watch-road line к northwatch как continuation, откуда назад приходит живое arrival word.",
        "detail_note": "Северная боковая линия здесь даёт уже не только confirmed arrival proof, а practical returned word: yard слышит, что resumed traffic по watch-road leg действительно дошёл дальше, и делает этот roadside ход честно returned-word line, а не только outward continuation.",
        "service_note": "После arrival-word follow-up waystation_yard ведёт watch-road line уже как line with returned living word, а не только как arrival-confirmed continuation.",
    },
    {
        "node_id": "ash_pass",
        "state_flag": "northwatch_directive_fulfilled",
    ...
    {
        "node_id": "ash_pass",
        "state_flag": "northwatch_marsh_edge_return_word_caught",
        "context_note": "На ash_pass уже держат мокрую линию к deep_marsh как edge-return continuation, откуда назад перехватывают живое return word.",
        "detail_note": "Край прохода теперь даёт уже не только confirmed arrival proof, а practical returned word: wet boundary leg слышит, что steadied traffic действительно влилось дальше в watch continuity, и делает edge-return line честно живой возвращающейся boundary line, а не только outward continuation.",
        "service_note": "После arrival-word follow-up ash_pass ведёт мокрую boundary line уже как line with caught return word, а не только как arrival-confirmed edge-return continuation.",
    },
    {
        "node_id": "deep_marsh_threshold",
    ...
    {
        "node_id": "blackwater_run",
        "state_flag": "deep_marsh_sidepass_arrival_word_gathered",
        "context_note": "У чёрной протоки уже держат cautious side-pass к western_road как continuation, откуда назад собирают живое arrival word.",
        "detail_note": "Боковая линия здесь даёт уже не только confirmed arrival proof, а practical returned word: reeds-side leg слышит, что guided traffic действительно прошло дальше по cautious pass, и делает этот marsh ход честно line with gathered back-word, а не только outward continuation.",
        "service_note": "После arrival-word follow-up blackwater_run ведёт cautious side-pass уже как line with gathered arrival word, а не только как arrival-confirmed forward continuation.",
    },
    {
        "node_id": "reed_shelter",
...
STATIC_MAP_NODE_ENTRY_OVERLAYS: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        "node_id": "northwatch_outpost",
        "first_entry_type": "settlement_welcome",
        "first_entry_title": "Северный рубеж встречает дозорным огнём",
        "first_entry_note": "Костёр, навесы и сторожевые силуэты сразу дают понять: здесь ещё держится живая линия обороны.",
        "return_entry_type": "return_entry",
        "return_entry_title": "Снова на северном рубеже",
        "return_entry_note": "Знакомый пост быстро возвращает чувство порядка: здесь всё ещё держат строй, считают припасы и следят за дорогой.",
    },
    {
        "node_id": "ash_pass",
        "first_entry_type": "quiet_entry",
        "first_entry_title": "Зольный проход открывается впереди",
        "first_entry_note": "За пределом поста воздух становится суше, пахнет гарью, а сама дорога кажется уже не линией дозора, а входом в тревожную землю.",
        "return_entry_type": "return_entry",
        "return_entry_title": "Снова в зольном проходе",
        "return_entry_note": "Гаревый ветер и тесный ход узнаются сразу, но даже знакомый путь здесь не обещает покоя.",
    },
    {
        "node_id": "deep_marsh_threshold",
        "first_entry_type": "quiet_entry",
        "first_entry_title": "Глубокие болота принимают неохотно",
        "first_entry_note": "Туман, мокрая тишина и редкие старые метки сразу подсказывают: дальше дорога держится не на уверенности, а на осторожности.",
        "return_entry_type": "return_entry",
        "return_entry_title": "Снова на пороге глубоких болот",
        "return_entry_note": "Первые сухие кочки ещё можно узнать, но дальше болото снова требует внимательности и коротких, точных решений.",
    },
    {
        "node_id": "blackwater_run",
        "first_entry_type": "quiet_entry",
        "first_entry_title": "Чёрная протока выходит из тумана",
        "first_entry_note": "Узкая тёмная вода сразу меняет весь характер пути: здесь болото уже не просто сырое, а по-настоящему опасное.",
        "return_entry_type": "return_entry",
        "return_entry_title": "Снова у чёрной протоки",
        "return_entry_note": "У тёмной воды по-прежнему мало места для ошибки, и даже знакомая переправа не кажется безопасной.",
    },
    {
        "node_id": "western_road_watch",
        "first_entry_type": "settlement_welcome",
        "first_entry_title": "Западный тракт снова полон движения",
        "first_entry_note": "Широкая дорога, следы обозов и свежая грязь на колее сразу дают понять: этот путь живёт, даже если живёт неспокойно.",
        "return_entry_type": "return_entry",
        "return_entry_title": "Снова на западном тракте",
        "return_entry_note": "Знакомая дорога читается легко, но следы недавней спешки всё ещё не дают считать её по-настоящему спокойной.",
    },
    {
        "node_id": "rutted_detour",
        "first_entry_type": "quiet_entry",
        "first_entry_title": "Разбитый объезд уводит в сторону",
        "first_entry_note": "Ровная дорога здесь заканчивается: дальше путь читается по разбитой колее, срывам хода и следам брошенного груза.",
        "return_entry_type": "return_entry",
        "return_entry_title": "Снова на разбитом объезде",
        "return_entry_note": "Здесь по-прежнему мало порядка и слишком много свежих следов, чтобы чувствовать себя спокойно даже на знакомом пути.",
    },
)
...
STATIC_MAP_REGION_ONBOARDING: tuple[dict[str, Any], ...] = (
    {
        ...
        "intel_summary": "При первом входе в стартовое пограничье группа быстро определяет основные безопасные выходы от стартового тракта.",
        "onboarding_note": "Стартовый тракт сразу задаёт понятные и безопасные направления внутри региона.",
    },
    {
        "region_id": "northwatch_frontier",
        "region_label": "Северный рубеж",
        ...
        "intel_summary": "При первом входе на северный рубеж группа сразу закрепляет пост, интендантский двор, обзорную палисаду и тревожный путь в зольный проход.",
        "onboarding_note": "Северный рубеж сразу показывает, где держится дозор, где можно перевести дух и откуда начинается опасная дорога дальше.",
    },
    {
        "region_id": "western_road",
        "region_label": "Западный тракт",
        ...
        "intel_summary": "При первом входе на западный тракт группа сразу закрепляет двор у дороги, каменную арку-ориентир и разбитый объезд со свежими следами недавнего прохода.",
        "onboarding_note": "Западный тракт сразу открывает понятные дорожные ориентиры: место для короткой остановки, заметную веху пути и рискованный обходной ход.",
    },
    {
        "region_id": "deep_marsh",
        "region_label": "Глубокие болота",
        ...
        "intel_summary": "При первом входе в глубокие болота группа сразу закрепляет тростниковый приют, старый путевой камень и опасный ход к чёрной протоке.",
        "onboarding_note": "Глубокие болота сразу показывают свой первый рисунок пути: редкое сухое убежище, старую примету направления и тёмную тропу в более опасную воду.",
    },
)
