...
def test_vedalken_tireless_precision_persists_selected_skill_and_tool(monkeypatch) -> None:
        ...
        http_routes.api_character_create(
            {
                ...
                "race_choices": {
                    "languages": ["elvish"],
                    "tireless_precision": {"skill": "arcana", "tool": "thieves_tools"},
                },
            }
        )
    )
    ...
    bonuses = race_features.get("bonuses") or {}
    tp_choice = choices.get("tireless_precision") or {}
    tp_bonus = bonuses.get("tireless_precision") or {}

    assert tp_choice.get("skill") == "arcana"
    assert tp_choice.get("tool") == "thieves_tools"
    assert tp_bonus.get("die") == "1d4"
    assert tp_bonus.get("skills") == ["arcana"]
    assert tp_bonus.get("tools") == ["thieves_tools"]


def test_vedalken_tireless_precision_choices_are_required(monkeypatch) -> None:
    _setup_create_mocks(monkeypatch)
    ...
    try:
        ...
        raise AssertionError("Expected HTTPException for missing Tireless Precision choices")
    except HTTPException as exc:
        assert exc.status_code == 400
        assert "Tireless Precision choices are required" in str(exc.detail)
