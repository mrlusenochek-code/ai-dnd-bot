...
from app.web.db_helpers import get_session
from app.web.session_state import _get_phase, settings_get


async def _read_session_runtime(session_id: str) -> dict[str, object]:
...
def test_e2e_player_journey_smoke(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with TestClient(server_mod.app) as client:
        ...
        with client.websocket_connect(f"/ws/{session_id}?uid={uid}&cid=e2e-smoke-a") as ws:
            ...
            begin_state = ws.receive_json()
            assert begin_state.get("type") == "state"
            assert (begin_state.get("game") or {}).get("phase") == "lore_pending"

        anyio.run(gm_orchestrator.run_lore_generation, session_id)
        post_lore = anyio.run(_read_session_runtime, session_id)
        assert post_lore["phase"] != "lore_pending", (
