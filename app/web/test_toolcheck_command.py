...
def test_parse_toolcheck_command_variants() -> None:
    assert ws_handlers._parse_toolcheck_command("toolcheck thieves_tools") == ("normal", "thieves_tools", None, False, None)
    assert ws_handlers._parse_toolcheck_command("toolcheck adv thieves_tools") == ("advantage", "thieves_tools", None, False, None)
    assert ws_handlers._parse_toolcheck_command("toolcheck dis thieves_tools dc 15") == ("disadvantage", "thieves_tools", 15, False, None)
    assert ws_handlers._parse_toolcheck_command("toolcheck pastlife herbalism_kit dc12") == ("normal", "herbalism_kit", 12, True, None)


def test_parse_toolcheck_command_reports_clear_errors() -> None:
    _mode, _tool, _dc, _pastlife, err_usage = ws_handlers._parse_toolcheck_command("toolcheck")
    assert err_usage is not None and "Использование" in err_usage

    _mode, _tool, _dc, _pastlife, err_dc = ws_handlers._parse_toolcheck_command("toolcheck thieves_tools dc")
    assert err_dc is not None and "dc" in err_dc.lower()

    _mode, _tool, _dc, _pastlife, err_bad_dc = ws_handlers._parse_toolcheck_command("toolcheck thieves_tools dc -1")
