...
FIGHTING_STYLE_CREATE_WHITELIST = {"defense", "archery", "dueling", "great_weapon_fighting"}
LANGUAGE_WHITELIST = {
    "common",
    "dwarvish",
    "elvish",
...
def _build_race_features(selected_race: dict | None) -> dict[str, Any]:
    ...
    for t in traits:
        ...
        if mtype == "damage_resistance":
            ...
            if tkey == "fire_resistance":
                damage_items = _uniq_lower_str_list(mech.get("damage"))
                if damage_type:
                    damage_items = _uniq_lower_str_list([*damage_items, damage_type])
                features["fire_resistance"] = {
                    "type": "damage_resistance",
                    "damage": damage_items,
                }
            if tkey == "mental_discipline":
                damage_items = _uniq_lower_str_list(mech.get("damage"))
                if damage_type:
                    damage_items = _uniq_lower_str_list([*damage_items, damage_type])
                features["mental_discipline"] = {
                    "type": "damage_resistance",
                    "damage": damage_items,
                }
            if tkey == "celestial_resistance":
                damage_items = _uniq_lower_str_list(mech.get("damage"))
                if damage_type:
                    damage_items = _uniq_lower_str_list([*damage_items, damage_type])
                features["celestial_resistance"] = {
                    "type": "damage_resistance",
                    "damage": damage_items,
                }
        ...
        if mtype == "save_advantage_conditions":
            conditions = _uniq_lower_str_list(mech.get("conditions"))
            save_advantage_conditions.extend(conditions)
            if tkey == "loxodon_serenity":
                features["serenity"] = {
                    "type": "save_advantage_vs_condition",
...
async def api_character_create(payload: dict):
    ...
    async with AsyncSessionLocal() as db:
        ...
        if isinstance(race_features, dict) and str(race_features.get("race_key") or "").strip().lower() == "hobgoblin":
            ...
            race_features["runtime"] = runtime
        if isinstance(race_features, dict) and str(race_features.get("race_key") or "").strip().lower() == "half_orc":
            runtime_raw = race_features.get("runtime")
            runtime = dict(runtime_raw) if isinstance(runtime_raw, dict) else {}
