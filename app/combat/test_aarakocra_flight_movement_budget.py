...
def test_aarakocra_flight_enters_combat_movement_speeds(monkeypatch) -> None:
    ...
    assert len(calls) == 1
    assert calls[0]["speed_ft"] == 25
    assert calls[0]["movement_speeds"]["walk"] == 25
    assert calls[0]["movement_speeds"]["fly"] == 50


def test_aarakocra_flight_blocked_in_heavy_armor_for_combat_movement(monkeypatch) -> None:
    calls: list[dict[str, Any]] = []
    ...
    assert len(calls) == 1
    assert calls[0]["movement_speeds"]["walk"] == 15
