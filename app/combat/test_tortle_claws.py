...
def test_tortle_claws_attack_has_named_log_and_damage(monkeypatch) -> None:
    session_id = "test_tortle_claws_attack"
    ...
    state.combatants["pc_1"] = Combatant(
        ...
        race_features={
            ...
            "natural_weapons": [{"key": "claws", "kind": "unarmed", "damage_dice": "1d4", "damage_type": "slashing", "ability": "str"}],
        },
    )
    ...
    try:
        patch, err = handle_live_combat_action("combat_tortle_claws", session_id)
        assert err is None and patch is not None
        texts = _line_texts(patch)
        assert any("Когти" in t for t in texts)
